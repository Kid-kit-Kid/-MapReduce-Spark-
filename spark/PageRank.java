...
//    conf.setMaster("local"); // 仅用于本地进行调试，如在集群中运行则删除本行
    ...
    // 读取输入文本
    JavaRDD<String> text = sc.textFile(args[2]);   // 修改输入路径
    ...
    long N = Long.parseLong(args[0]); // 获取网页总数
...
//    });
      // 获取输出目录路径参数
      String outputPath = args[1]; // 修改输出路径

      // 将排名结果保存到指定路径的文本文件中
      ranks.mapToPair(
                      new PairFunction<Tuple2<String, Double>, String, String>() {
                          @Override
                          public Tuple2<String, String> call(Tuple2<String, Double> t) throws Exception {
                              // 格式化每行输出
                              return new Tuple2<>(t._1, String.format("%.5f", t._2));
                          }
                      })
